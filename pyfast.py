...
# Global variables
#========================================================
fastra=str(106.85645657571428)
fastdec=str(25.6534387)
fastalt=str(1138.72178)
convert=np.pi/180.0
fastct = time.gmtime()
fastct3 = time.strftime("%Y/%m/%d %H:%M:%S",fastct)
fastjd=ephem.julian_date(fastct3)
fastyear = int(time.strftime("%Y",fastct))
fastmonth= int(time.strftime("%m",fastct))
#========================================================


# Functions from helio package
#========================================================
from numpy import *
...
def angdistance(ra1,dec1,ra2,dec2):
    global fastra,fastdec,fastalt,convert
    ra1temp=ephem.hours(str(ra1))
    dec1temp=ephem.degrees(str(dec1))
    ra2temp=ephem.hours(str(ra2))
    dec2temp=ephem.degrees(str(dec2))

    body1 = ephem.FixedBody()
    body1._ra = ra1temp
    body1._dec = dec1temp
    body1._epoch = ephem.J2000
    body1.compute()
    body2 = ephem.FixedBody()
    body2._ra = ra2temp
    body2._dec = dec2temp
    body2._epoch = ephem.J2000
    body2.compute()
    print "%f degrees"%(float(ephem.separation(body1,body2))/convert)
    return float(ephem.separation(body1,body2))/convert

def azel2glongglat(az=90,el=20):
    global fastra,fastdec,fastalt,convert
    ra0,dec0=azel2radec(az,el)
    eb=ephem.Equatorial(ephem.degrees(ra0),ephem.degrees(dec0),epoch=ephem.J2000)
    gb=ephem.Galactic(eb)
    print gb.lon, gb.lat
    return gb.lon, gb.lat
    
def radec2azel(ra=20,dec=20):
    global fastra,fastdec,fastalt,convert
    getlocal = ephem.Observer()
    getlocal.long, getlocal.lat = fastra,fastdec
    ct = time.gmtime()
...
def constants():
    print 'Sun Radius',ephem.sun_radius
    print 'Moon Radius',ephem.moon_radius
    print 'Earth Radius',ephem.earth_radius
    print 'B1900',ephem.B1900
    print 'B1950',ephem.B1950
    print 'J2000',ephem.J2000
    print 'Meters per au',ephem.meters_per_au
    print 'Speed of Light',ephem.c

def date2jd(date):
    jd=ephem.julian_date(str(date))
    jdstr='%.6f'%(jd)
    print jdstr
    return jdstr
...
def jd2date(jd):
    ...
    return ephem.Date(djd)

def j2000tojnow(ra,dec):
    ra0=ephem.degrees(ephem.hours(str(ra)))
    dec0=ephem.degrees(str(dec))
...
def deg2degstr(deg):
    decd = int(deg)
    decm = int((deg-decd)*60)
    decs = (deg-decd-decm/60.0)*3600
    if (decd<10):
       strdecd=str(0)+str(decd)
    else:
       strdecd=str(decd)
    if (decm<10):
       strdecm=str(0)+str(decm)
    else:
       strdecm=str(decm)

    strdecs='%.2f'%(decs)

    degstr=strdecd+':'+strdecm+':'+strdecs
...
def azel2radec(az,el):
    global fastra,fastdec,fastalt,convert
    getlocal = ephem.Observer()
    getlocal.long, getlocal.lat = fastra,fastdec # at Dawodang
...
def local2ec(az,el):
    return azel2radec(az,el)


def xyz2radec(x0,y0,z0):
    ...
    if (np.abs(x) < 1.0-8):
       ...
       if ((x<0 and y<0) or (x<0 and y==0)):
           az0 = tempaz0+180.0
    el0 = np.arcsin(z/R)/convert
    ra,dec=azel2radec(az0,el0)
    return ra,dec

def obj_transit(ra,dec):
# ra and dec should be strings! 
# Number also accepted. 20161120
    global fastra,fastdec,fastalt,convert
    ra0=ephem.hours(str(ra))
    dec0=ephem.degrees(str(dec))
    getlocal = ephem.Observer()
    getlocal.long, getlocal.lat = fastra,fastdec # at Dawodang
    getlocal.horizon='60' # Astronomical twilight uses the value -18 degrees
                          # FAST zenith angle 30 degrees, 90-30=60
    ct = time.gmtime()
    ct3 = time.strftime("%Y/%m/%d %H:%M:%S",ct)
    getlocal.date = ct3 # UT
    ...
    body._epoch = ephem.J2000
    body.compute(getlocal)
    trise=getlocal.previous_rising(body)
    print 'previous rise'
    print body.az,body.alt,ephem.localtime(trise)
    ttrans=getlocal.next_transit(body)
    print 'next transit'
    print body.az,body.alt,ephem.localtime(ttrans)
    tset=getlocal.next_setting(body)
    print 'next setting'
    print body.az,body.alt,ephem.localtime(tset)


def lst():
    global fastra,fastdec,fastalt,convert
    Dawodang=ephem.Observer()
    Dawodang.lat=fastdec
    Dawodang.lon=fastra
    ct = time.localtime()
    ct3 = time.strftime("%Y/%m/%d %H:%M:%S",ct)
    print('Beijing Time: ',ct3)
    print('Sideral Time: ',Dawodang.sidereal_time())

def ut2lst(ct=fastct3):
    global fastra,fastdec,fastalt,convert
    Dawodang=ephem.Observer()
    Dawodang.lat=fastdec
    Dawodang.lon=fastra
...
def showcalendar(year=fastyear,month=fastmonth):
    cal = calendar.month(year,month)
    print cal

def vcorrection_bary(ra='05:34:32.0',dec='+22:01:10'):
    fastct = time.gmtime()
    fastct3 = time.strftime("%Y/%m/%d %H:%M:%S",fastct)
    fastjd=ephem.julian_date(fastct3)
    ra2000=ephem.hours(str(ra))
    dec2000=ephem.degrees(str(dec))
    vcorrection,hjd = helcorr(float(fastra), float(fastdec), float(fastalt), ra2000, dec2000, fastjd, debug=False)
    return vcorrection

def vcorrection_lsr(ra='05:34:32.0',dec='+22:01:10'):
    fastct = time.gmtime()
    fastct3 = time.strftime("%Y/%m/%d %H:%M:%S",fastct)
    fastjd=ephem.julian_date(fastct3)
    ra2000=ephem.hours(str(ra))
    ...
    eb=ephem.Equatorial(ra2000,dec2000,epoch=ephem.J2000)
    gb=ephem.Galactic(eb)
    vlsr_vbsr=9.0*np.cos(gb.lon)*np.cos(gb.lat)+12.0*np.sin(gb.lon)*np.cos(gb.lat)+7.0*np.sin(gb.lat)
    return vcorrection+vlsr_vbsr
...
def nowjd():
    ...
    date=ephem.julian_date(ct3)
    print date

    print ephem.localtime(ephem.Date(ct3))
    print date+8.0/24.0


def parallacticangle(ra,dec):
    az,el = radec2azel(ra,dec)
    convert=np.pi/180.0
    pa=np.arcsin(np.sin(az*convert)*np.cos(float(Dawodang.dec)*convert)/np.cos(dec*convert))/convert
    if(pa<0):
       pa=pa+180.0
    return pa
...
def plotmultibeam(x,y,angle):
    ...
    for i in range(1,7,1):
        circtemp=Circle(xy=(x0+d*np.cos(np.pi/3.0*i+angle),y0+d*np.sin(np.pi/3.0*i+angle)),radius=beam/2,)
        circle.append(circtemp)
    
    d2=np.sqrt((d*np.cos(np.pi/3.0)+d)**2+(d*np.sin(np.pi/3.0))**2)
    for i in range(1,7,1):
        circtemp=Circle(xy=(x0+d2*np.cos(np.pi/3.0*(i-1)+np.pi/6.0+angle),y0+d2*np.sin(np.pi/3.0*(i-1)+np.pi/6.0+angle)),radius=beam/2,)
        circle.append(circtemp)
    
    d3=d*2
    for i in range(1,7,1):
        circtemp=Circle(xy=(x0+d3*np.cos(np.pi/3.0*i+angle),y0+d3*np.sin(np.pi/3.0*i+angle)),radius=beam/2,)
        circle.append(circtemp)
...
def disp_continuum(filename='FP160606_RFI2_0001.fits',lchan=550,uchan=650):
    ...
    widthfreq=header0['OBSBW']
    nchan=header0['OBSNCHAN']
    
    timespec4D=data1['DATA']
    nsblk=header1['NSBLK']
    nspec=size(timespec4D)/nchan/nsblk/2
    tsample=header1['TBIN']
    
    nt=nsblk
    timespec2D=np.zeros([nchan,nt])
    timespec1D=np.zeros(nspec)

    x=np.array(range(nspec))*nt*tsample # seconds
    
    for i in range(nspec):
        for j in range(nt):
            timespec1D[i]=timespec1D[i]+sum(timespec4D[i,j,1,lchan:uchan,0])*1.0
        timespec1D[i]=timespec1D[i]/nt
        #print x[i],timespec1D[i]
    ...
    ax=fig.add_subplot(111)
    ax.plot(x,timespec1D)
    
    ylabel('Intensity',fontdict=font)
    xlabel('Time (s)',fontdict=font)
    
    #savefig("FAST_continuum.png")
    plt.show()

def disp_spec(filename='FP160606_RFI2_0001.fits',timetick=100,subinttick=0):
    hdulist = pyfits.open(filename)
    hdu1 = hdulist[1]
    data = hdu1.data
    header = hdu1.header
    
    timespec4D=data['DATA']
    spec1D=timespec4D[timetick,subinttick,0,:,0]
...
def extract_spec(filename='FP160606_RFI2_0001.fits',timetick=100,subinttick=0):
    hdulist = pyfits.open(filename)
    hdu1 = hdulist[1]
    data = hdu1.data
    header = hdu1.header
...
def fit_Ngaussian(freq,spec,ngaussian):
    createVar=locals()
    ...
    init = mod.eval(pars,x=freq)
    out = mod.fit(spec,pars,x=freq)
    para = np.zeros([ngaussian,3])
    for i in range(0,ngaussian):
        para[i,0]=out.params['g'+str(i)+'_amplitude'].value
        para[i,1]=out.params['g'+str(i)+'_center'].value
        para[i,2]=out.params['g'+str(i)+'_sigma'].value
    ...
    plt.plot(freq,residual,'r')
    #plt.plot(freq,spec-residual,'b')
    plt.plot(freq,spec+residual,'b')
    plt.show()
...
def fit_base(freq,spec,npoly,window):
    createVar=locals()
    nwindow=np.size(window)
    freqfit=freq[freq<window[0]]
    specfit=spec[freq<window[0]]
    if(nwindow>2):
        for i in range(nwindow-1):
            freqtemp=freq[freq>window[i]]
            freqtemp2=freqtemp[freqtemp<window[i+1]]
            spectemp=spec[freq>window[i]]
            spectemp2=spectemp[freqtemp<window[i+1]]
            freqfit=np.concatenate((freqfit,freqtemp2))
            specfit=np.concatenate((specfit,spectemp2))
    freqtemp2=freq[freq>window[-1]]
    spectemp2=spec[freq>window[-1]]
    freqfit=np.concatenate((freqfit,freqtemp2))
    specfit=np.concatenate((specfit,spectemp2))
    #print specfit
    my_model = Model(func_poly)
    params = Parameters()
    for i in range(npoly):
        paraname='C'+str(i)
	params.add(paraname,value=1.0+np.random.rand())
    out = my_model.fit(specfit,params,x=freqfit)
    residual=out.residual
    print out.params
    #print(out.fit_report())
    plt.plot(freq,spec,'k')
    plt.plot(freqfit,residual,'r')
    #plt.plot(freq,spec-residual,'b')
    plt.plot(freqfit,specfit+residual,'b')
    plt.show()
